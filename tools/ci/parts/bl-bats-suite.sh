...
do
  ...
  test -e $baseline_d$baseline$baseline_ext || {
    print_yellow "" "No baseline $baseline, skipped"
    continue
  }

  print_yellow "" "Starting $baseline baseline..."
  bats $baseline_d$baseline$baseline_ext &&
    print_green "OK" "$baseline baseline" ||
    print_red "Not OK" "$baseline baseline"
